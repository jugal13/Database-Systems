...
SELECT Sid FROM SUPPLIER WHERE Sid IN (SELECT Sid FROM SHIPMENT WHERE Pid IN (SELECT Pid FROM PART WHERE Color = "Green"));
-- or
...
-- or
UPDATE PART SET Color='Yellow' WHERE Pid IN (SELECT Pid FROM SHIPMENT WHERE Sid IN (SELECT Sid FROM SUPPLIER WHERE Sid=3));